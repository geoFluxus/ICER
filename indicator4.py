...
viz_data = pd.concat([province_data, ind_values_colour], axis=1)
viz_data = viz_data.reset_index()
viz_data.columns = ['CRM', 'DMI', indicator_colour]
viz_data['label'] = viz_data.apply(lambda x: '<br>'.join([f'{name}: {x[name]}' for name in viz_data.columns]), axis=1)

viz_data.sort_values('DMI', inplace=True)
...
transp_indicators[indicator_production] = transp_indicators[indicator_production] * 10 / max_production

# Recycling rate USGS, %
indicator_recycling = indicator_list[3]
transp_indicators[indicator_recycling] = transp_indicators[indicator_recycling]/10
